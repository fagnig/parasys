 ...
 *  Free to good home!
 *
 * MODIFIED BY ERIK TO READ START LOCATION OF PIXEL DATA FROM THE BMP HEADER 
 * SINCE THIS FILE ASSUMED NO ENCODING
 */
...
int ImageLoad(char *filename, Image *image) {
	...
	/* MODIFICATION BY ERIK */
    fseek(file, 0xA, SEEK_SET);
	int start_of_pixel_array = endianReadInt(file);
   ...
   // printf("Height of %s: %lu\n", filename, image->sizeY);
	...
	/*modification by erik*/
    fseek(file, start_of_pixel_array, SEEK_SET);
